...
1. 
    * 词法作用域每次函数求值时都创建一个新的env，包含了函数定义时所能访问到的各种binding。这个新的env连同那个函数一起，俗称闭包closure。
    * 动态作用域在程序运行时只有一个env(env是一组identifer到value的binding)，在每次的函数求值时，只会在这个唯一的env里查询或更新。
2. 
