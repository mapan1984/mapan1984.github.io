...
`tailrec` 可以标记在你认为时尾递归的递归函数中，如果编译器无法对它做尾递归优化，系统将抛出异常。
...
惰性赋值：求值只会在需要是进行，而且只执行一次（因此 `lazy` 不会修饰 `var`，只会修饰 `val`）
...
在 `case` 字句中，以小写字母开头的标识符被认为时用来提取待匹配值的新变量。如果需要引用之前已经定义的变量时，应使用反引号将其包围。与此相对，以大写字母开头的标识符被认为时类型名称。
...
**纯函数**：在数学里，函数没有副作用。一下是经典的 `sin(x)` 函数：

```
y = sin(x)
```

无论 `sin(x)` 做了多少计算，它的所有结果豆瓣函数返回并赋值给了 `y`。在 `sin(x)` 内部，没有任何全局状态被修改。这样，我们就称改函数是无副作用函数，即纯函数。

---

当一个函数采用其他函数作为变量或返回值是，它被称为**高阶函数**。在数学里，微积分中有两个高阶函数的例子————微分和积分。
...
**偏应用函数**表示表达式中使用了函数，但并未给出所需的所有参数列表。所以，系统返回了一个新的函数，该函数的参数列表时原函数中没有给出的剩下的那部分参数列表。
...
派生类的主构造器必须调用父类的构造器，可以时父类的主构造器或次级构造器。
