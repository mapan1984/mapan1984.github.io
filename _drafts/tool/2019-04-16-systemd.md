...
* 如果 shell script 执行不会结束，例如无限循环，启动进程为主进程，则设置类型为 `simple`
* 如果 shell script 会创建一个子进程在后台执行，自身退出，创建的自进程为主进程，则设置类型为 `forking`。对于 `forking` 类型的 service，通常还要通过 `PIDFile=` 指定自进程的进程 id。
* 如果 shell script 执行后就结束，不需要长期运行的进程，则设置类型为 `oneshot`。可以搭配 `RemainAfterExit=yes`
* `dbus`
* `notify`
* `idle`
...
关闭自启动：
    ...
    Removed /etc/systemd/system/default.target.wants/zookeeper.service.
