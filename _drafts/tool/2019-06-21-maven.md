...
* `-pl <module> -am`
    * `-pl` `--projects`: 只构建指定的 module
    * `-am` `--also-make`: 除了构建指定的模块，还构建指定模块所依赖的其他模块
...
执行单元测试并生成报告（报告仅显示测试结果，即通过、失败、错误）
    ...
    <plugins>
        ...
        <plugin>
            ...
            <executions>
                ...
                <!-- Generate report after tests have run -->
                <execution>
                    <id>report</id>
                    <phase>verify</phase>
                    <goals>
...
执行 `mvn clean verify`，在 `target/site/jacoco` 目录下查看 HTML 报告
