...
* -s number: 指定截取的报文字节数，tcpdump 默认只会截取前 96 字节的内容，如果 number 指定为 0，表示截取报文全部内容。
* -n 表示不要解析域名，直接显示 ip。
* -nn 不要解析域名和端口
...
来自 10.124.236.101 的请求

    tcpdump -nn host 10.124.236.101
...
kafka 数据
    ...
    tcpdump -i eth0 -s 0 -w kafka_9092_clients.pcap

    tcpdump -i eth0 -s 0 -w kafka_9092_%Y%m%d%H%M%S.pcap -G 300 -W 100 '(host 10.81.71.14 or host 10.81.73.32) and port 9092'
