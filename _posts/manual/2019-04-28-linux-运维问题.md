...
1. SYN Queue 半连接队列: tcp 服务端收到第一个 syn 时，连接处于 SYN_RCVD 状态，并加入到 syn queue 中。随后 tcp 服务端响应 syn,ack，此时 socket 被称为半连接 socket
2. Accept Queue 全连接队列: 当服务端收到响应的 ack 后，连接处于 ESTABLISHED 状态，将连接从 syn queue 移除，加入到 accept queue。此时 socket 为已连接 socket，服务端阻塞的 `accept()` 函数会从 accept queue 取 established socket。
